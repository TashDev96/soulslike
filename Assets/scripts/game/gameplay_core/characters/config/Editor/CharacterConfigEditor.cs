...
namespace game.gameplay_core.characters.config.Editor
{
	...
	public class CharacterConfigEditor : OdinEditor
	{
		...
		{
			if(thisObj == null || parentObj == null)
			{
				return;
			}
			if(thisObj.GetType() != parentObj.GetType())
			{
				return;
			}
		...
		private bool IsInlineSerializedType(Type type)
		{
			if(type == null)
			{
				return false;
		...
		private bool AreValuesEqual(object value1, object value2)
		{
			...
			if(value1 is Object unityObj1 && value2 is Object unityObj2)
			{
		...
		private bool AreAnimationCurvesEqual(AnimationCurve curve1, AnimationCurve curve2)
		{
			...
			for(var i = 0; i < curve1.keys.Length; i++)
			{
		...
		private object GetNestedValue(object obj, string[] fieldParts)
		{
			...
			foreach(var part in fieldParts)
			{
				...
				throw new Exception($"Field or property '{part}' not found");
			}
			return current;
		}
		...
		private void SetNestedValue(object obj, string[] fieldParts, object value)
		{
			if(fieldParts.Length == 1)
			{
				...
				throw new Exception($"Cannot set field or property '{fieldParts[0]}'");
			}
