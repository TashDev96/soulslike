...
public static class TargetingManager
	{
		...
		private static readonly Dictionary<ZombieUnit, HashSet<TowerUnit>> zombieTargeters = new();
	...
	private static int validationCacheFrame = -1;
		...
		private static void GetNeighborGridKeys(Vector3 center, float radius, HashSet<int> keys)
		{
			...
			int centerX = Mathf.FloorToInt(center.x / gridCellSize);
			int centerZ = Mathf.FloorToInt(center.z / gridCellSize);
		...
		private static void UpdateSpatialGrid()
		{
			...
			foreach (var zombie in cachedAliveZombies)
			{
				...
				{
					int key = GetGridKey(zombie.Position);
		...
		private static bool IsZombieValidCached(ZombieUnit zombie)
		{
			...
			if (zombieValidationCache.TryGetValue(zombie, out int lastValidFrame))
			{
				return lastValidFrame == Time.frameCount && !zombie.IsDead;
			}
			
			bool isValid = !zombie.IsDead;
			if (isValid)
			{
				zombieValidationCache[zombie] = Time.frameCount;
			}
		...
		private static bool IsZombieValidForTower(ZombieUnit zombie, TowerUnit tower)
		{
			...
			return zombie.HasPendingDamageFrom(tower);
		}
	...
	public static ZombieUnit RequestTarget(TowerUnit tower, float maxRange)
	{
		...
		foreach (int key in tempGridCells)
		{
			if (spatialGrid.TryGetValue(key, out var zombiesInCell))
			{
				for (int i = 0; i < zombiesInCell.Count; i++)
				{
		...
		int leastTargeters = int.MaxValue;

		for (int i = 0; i < tempTargetsInRange.Count; i++)
		{
			var zombie = tempTargetsInRange[i];
			int targeterCount = GetTargeterCount(zombie);
		...
		{
			return zombie != null && zombieTargeters.ContainsKey(zombie) && zombieTargeters[zombie].Count > 0;
		}

		public static int GetTargeterCount(ZombieUnit zombie)
		{
			return zombie != null && zombieTargeters.TryGetValue(zombie, out var targeters) ? targeters.Count : 0;
		}
		...
		public static void ClearTowerTarget(TowerUnit tower)
		{
			...
			if (towerTargets.TryGetValue(tower, out var currentTarget))
			{
				...
				if (currentTarget != null && zombieTargeters.TryGetValue(currentTarget, out var targeters))
				{
					targeters.Remove(tower);
		...
		public static void OnZombieDied(ZombieUnit zombie)
		{
			...
			if (zombieTargeters.TryGetValue(zombie, out var targeters))
			{
				foreach (var tower in targeters.ToList())
				{
					towerTargets.Remove(tower);
				}
		...
		{
			if (tower == null || zombie == null) return;
			...
			if (!zombieTargeters.TryGetValue(zombie, out var targeters))
			{
				targeters = new HashSet<TowerUnit>();
				zombieTargeters[zombie] = targeters;
			}
			targeters.Add(tower);
		}
